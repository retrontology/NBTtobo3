...
def convertNBTToBlock(tag):
	block = convertNameToID(tag['Name'].value)
	if(tag.__contains__('Properties')):
		data = 0
		if(tag['Properties'].__contains__('facing')):
			data = data + convertDirectionToNum(tag['Properties']['facing'].value)
		block = block + ":" + str(data)
	return(block)

def convertDirectionToNum(direct):
	return {
		'down':0 , 
		'up':1 , 
		'north':2 , 
